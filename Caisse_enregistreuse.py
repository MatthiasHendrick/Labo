...
###FUNCTION TICKET###
def purchaseDB(entry_employee_id, entry_menu_id = 0):
  purchase_id = get_purchase_id()
  current_date = datetime.datetime.now()
  formated_date = current_date.strftime("%d-%m-%Y %H:%M:%S")

  employee_id = get_employee_id(entry_employee_id)
  ...
  while True:
    ...
    afficher_ticket = display_ticket(ticket_data)
    output = ''
    for item in afficher_ticket :
        output = output + '\n' + item
    ecranticket.config(text = f'{output}')
    print(afficher_ticket)
    print(type(afficher_ticket))
    break
...
def display_ticket(ticket_data):
  ...
  myListe.append(purchase_line)
  myListe.append(date_line)
  myListe.append(employee_line)
  ...
  for index in range(3,len(ticket_data)): #[purchase_id,date,employee,(p_id,menu_id,price)]
    ...
    print(detail_line)
    myListe.append(detail_line)
    amount = amount + price #TODO pourquoi 3.3 * 3 donne 9.89999999?
  amount_line = 'Amount : ' + str(amount) + ' €'
  print(amount_line)
  myListe.append(amount_line)
  print('----------')
  print(myListe)
...
def get_employee_id(entry_employee_id): #TODO idem que get_menu_id()?
...
def create_fen_purchase():
    ...
    def create_fen_purchase_menu():
        ...
        purchase_menu.title('PURCHASE MENU')
        purchase_menu.configure(bg = 'grey')
        purchase_menu.wm_attributes("-topmost", 1)

        def get_entry_menu() :

            entry_menu_id2 = menu_id.get()
            entry_menu_id = int(entry_menu_id2)

            #ecranticket.config(text = f'{entry_menu_id}')
            #purchaseDB(entry_menu_id)
            print(backup_empl_id)
            affichage = purchaseDB(backup_empl_id, entry_menu_id)
        ...
        ("Arial", 12, "roman","bold"), width = 12, height = 1, command = get_entry_menu)
        benter.grid(row = 2, column = 4, sticky = 'w', padx = 5)

        purchase.wait_window(purchase)

    def get_entry_employee() :
        global backup_empl_id
        int_entry_employee_id2 = employee_id.get()
        entry_employee_id = int(int_entry_employee_id2)
        #ecranticket.config(text = f'{entry_employee_id}')
        #purchaseDB(entry_employee_id)
        print(entry_employee_id)
        print(type(entry_employee_id))
        backup_empl_id = entry_employee_id
        create_fen_purchase_menu()
    ...
    ("Arial", 12, "roman","bold"), width = 12, height = 1, command = get_entry_employee)
    benter.grid(row = 2, column = 4, sticky = 'w', padx = 5)
...
###CANVAS TICKET###
ecranticket = Label(fen_principale, width = 68, height = 35, bg = 'ivory', font =
("Arial", 12, "roman","bold"), anchor = "nw")
